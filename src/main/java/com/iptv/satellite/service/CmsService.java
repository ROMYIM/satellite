			...
			if (epgs != null && epgs.size() > 0) {                                            //判断要插入数据的数量
				...
				if (scheduleBeans.size() > eachInsertCount) {                              //插入数目大于每次的插入数目就间隔插入
					...
					for (int j = 0; j < scheduleBeans.size() / eachInsertCount + 1; j++) {          //防止不能整除，所以总循环加一
						...
						insertCount += scheduleDAO.insertIntoCmsFromSchedule(scheduleBeans.subList(insertCount, count));     //插入新数据并累加数目
					}
					log.setInsertCount(insertCount);                                        //把插入数目写入日志
				} else {
					log.setInsertCount(scheduleDAO.insertIntoCmsFromSchedule(scheduleBeans));   //直接插入并添加到日志
				}                                         
			}
		} catch (Exception e) {
			e.printStackTrace();
